...
(function () {
  ...
  function GetVariable(server, name) {
    return global.Nog.Helpers.GetPersistentValue(
      server, DAYCARE_VARIABLES, VARIABLE_PREFIX, name,
    );
  }

  function SetVariable(server, name, value) {
    return global.Nog.Helpers.SetPersistentValue(
      server, DAYCARE_VARIABLES, VARIABLE_PREFIX, name, value,
    );
  }

  function GetDebugger(server, name) {
    return global.Nog.Helpers.GetPersistentValue(
      server, DEBUGGERS, DEBUGGER_PREFIX, name,
    );
  }

  function SetDebugger(server, name, value) {
    return global.Nog.Helpers.SetPersistentValue(
      server, DEBUGGERS, DEBUGGER_PREFIX, name, value,
    );
  }
    ...
    if (GetDebugger(server, "walk")) {
      Tell(
        ...
        "Distance since last position: " + distance.toFixed(1).replace(".", ",") +
          " | XP generated: " + generatedXp +
          " | XP applied: " + appliedXp +
          " | Hypothetical XP: " + pendingXp +
          " | XP per Pokemon: " + plan.xpPerPokemon +
  ...
  ServerEvents.tick(function (event) {
    ...
    while (players.hasNext()) {
      try { HandlePlayerInterval(players.next(), event.server); }
      catch (error) { global.Nog.Helpers.LogError(LOG_TAG, "player interval", error); }
    }
  });

  ServerEvents.loaded(function (event) {
    let players = event.server.getPlayerList().getPlayers().iterator();
    while (players.hasNext()) global.Nog.CobblemonDaycare.SubscribePlayer(players.next(), STATE);
  });
  ...
  function RunMyPasturedCobblemons(context) {
    ...
    for (let index = 0; index < ids.length; index++) {
      ...
      Tell(player, "[" + global.Nog.Cobblemon.GetPokemonName(pokemon) +
        "] Lv [" + Number(pokemon.getLevel()) + "]");
      displayed++;
    }
    if (displayed === 0) Tell(player, "You have no Pokemon currently on a Pasture.");
    return 1;
  }
  ...
  function RunDebuggerStatus(context) {
    ...
    for (let index = 0; index < names.length; index++) {
      ...
      Tell(player, names[index] + ": " + (enabled ? "active (true)" : "inactive (false)"));
    }
    return 1;
  }
  ...
  function CreateSetDebugger(name) {
    return function (context) {
      ...
      Tell(player, name + " is now " + (enabled ? "active (true)." : "inactive (false)."));
      return 1;
    };
  }
  ...
  ServerEvents.commandRegistry(function (event) {
    ...
    let debuggerCommand = Commands.literal("debuggers_status")
      .executes(RunDebuggerStatus).then(debuggerSet);
    ...
    for (let index = 0; index < variableNames.length; index++) {
      ...
      let valueArgument = definition.type === "boolean"
    ...
    event.register(
      Commands.literal("daycarexp")
        ...
        .then(Commands.literal("mypasturedcobblemons").executes(RunMyPasturedCobblemons))
